            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-x-4 sm:gap-x-6 gap-y-3">
              {metroCities.map((city) => (
                <Link
                  key={city.name}
                  to={
                    ['dallas-fort-worth', 'jacksonville', 'atlanta'].includes(city.slug)
                      ? `/locations/${city.slug}`
                      : '/quote'
                  }
                ...
                >
                  <MapPin size={14} className="text-secondary-300 group-hover:text-brand transition-colors duration-200 shrink-0" />
                  <span>{city.name}</span>
                </Link>
              ))}
            </div>

-- Reliable driver profile fetch: auto-links CRM rows by email, then returns the driver's row.
CREATE OR REPLACE FUNCTION public.get_my_driver()
...
$$;
REVOKE ALL ON FUNCTION public.get_my_driver() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_my_driver() TO authenticated;

-- Do not create a throwaway pending driver when a CRM admin opens the driver app.
...
BEGIN
  ...
  IF NOT FOUND THEN
    IF public.is_crm_admin() THEN
      RAISE EXCEPTION 'No driver profile linked to this account. Use your driver email, not an admin login.';
    END IF;
